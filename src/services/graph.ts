...
export async function fetchProjects(msal: IPublicClientApplication): Promise<Project[]> {
  ...
  return items
    ...
    .map((item: any): Project => ({
      ...
      projectManagerName:item.fields.ProjectManager?.displayName ||item.fields.ProjectManager?.Title ||item.fields.ProjectManager?.LookupValue ||item.fields.ProjectManagerEmail?.split('@')[0],
      techLeadEmail: item.fields.TechLeadEmail ?? '',
      techLeadName: item.fields.TechLead?.displayName ||item.fields.ProjectManager?.Title ||item.fields.ProjectManager?.LookupValue ||item.fields.ProjectManagerEmail?.split('@')[0],
